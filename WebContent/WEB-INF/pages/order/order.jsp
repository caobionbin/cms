...
<body class="">
				...
				<div class="modal  fade" id="addModal" role="dialog"
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<form action="/webtest/orders/addOrder" method="post"
									...
									<div class="form-group">
										...
										<div
											...
											<input type="text" id="orderTimeid" name="orderTime"
												class="form-control" size="16" readonly="readonly" /> <span
					...
					<div class="col-md-offset-11">
						<div class="col-md-1 ">
							<div class="btn-toolbar">
								<a href="#addModal" role="button" class="btn btn-primary"
							...
							<table
								...
								<c:forEach items="${orders}" var="tbl">
									<tr>
										...
										<td>${tbl.orderTime}</td>
										<td>
											<!-- 怎么实现的？ --> <c:set var="totalPrice" value="${0}"></c:set>
											<c:forEach items="${tbl.orderDetails}" var="od_list">
												<c:set var="totalPrice"
													value="${totalPrice+ (od_list.menus.price*od_list.num)}" />
											</c:forEach> ${totalPrice}
										</td>
										<td><c:if test="${tbl.idpay==1}">
												<div>
													<input type="checkbox" checked data-on-text="已结账"
														data-off-text="未结账" readonly="readonly">
												</div>
											</c:if> <c:if test="${tbl.idpay==0}">
												<div>
													<input type="checkbox" data-on-text="已结账"
														data-off-text="未结账" readonly="readonly">
												</div>
											</c:if></td>
										<td>${tbl.remark }</td>
										<td><a
											href="/webtest/orders/getOneUpdateOrder?id=${tbl.id}"
											class="btn btn-primary"><i
												class="glyphicon glyphicon-pencil"></i> 修改</a>
											<button onclick="del(this)" delid="${tbl.id}"
												pNow="${nowpage}" class="btn btn-danger">
							...
							<div class="col-md-offset-4">
								<ul class="pagination">
									<c:if test="${hasPre == true}">
										<li><a
											href="/webtest/orders/getOrders?nowpage=${nowpage-1 }">上一页</a></li>
									</c:if>
									<c:if test="${nowpage > 3 }">
										<c:if test="${totalPage-nowpage < 3 }">
											<c:forEach var="i" begin="${nowpage-3 }" end="${totalPage }">
												<li><a href="/webtest/orders/getOrders?nowpage=${i }">${i }</a>
												</li>
											</c:forEach>
										</c:if>
										<c:if test="${totalPage-nowpage >= 3 }">
									...
									<c:if test="${hasNext == true }">
										<li><a
											href="/webtest/orders/getOrders?nowpage=${nowpage+1 }">下一页</a></li>
									</c:if>
								</ul>
							</div>
						</div>
	...
	<script type="text/javascript">
		...
		function del(e) {
			...
			var pageid=$(e).attr("pNow");
			//var pNow = $(e).attr("pNow");
			if (confirm("确定删除？")) {
				$
						.ajax({
							type : "get",
							url : "/webtest/orders/deleteOrder?id=" + delid,
							dataType : "json",
							success : function(result) {
								if (result != null && result != "") {
									document.forms[0].action = "/webtest/orders/getOrders?nowpage="+pageid;
