					...
					<div class="col-md-offset-11">
						<div class="col-md-1 ">
							<div class="btn-toolbar">
								<a href="/webtest/menus/getOneMenuAdd" role="button"
					...
					<div class="row-fluid">
						<%-- <div class="row-fluid">
							<div class="btn-toolbar col-md-12 well">
								<a href="/webtest/menus/getOneMenuAdd" role="button" class="btn btn-primary" >
								...
								<div class="row-fluid">
									<c:forEach var="menu" items="${menus }" begin="0" end="3">
										<li class="thumbnail col-md-3">
											<div>
												<img alt="yyy" src="<%=basePath %>images/${menu.imgName }"
													width="220" height="170">
												<h3>${menu.name }</h3>
												<!-- nowpage没加 -->
												<a href="/webtest/menus/getPreMenu?id=${menu.id }"
													class="btn btn-primary">预览</a> <a class="btn btn-info"
													href="/webtest/menus/getOneMenu?id=${menu.id }"> <i
													class="glyphicon glyphicon-pencil"></i> 修改
												</a> <a onclick="del(this)" class="btn btn-warning"
													delid="${menu.id }" pNow="${nowpage }"> <i
							...
							<div class="col-md-offset-4">
								<ul class="pagination">
									<c:if test="${hasPre == true}">
										<li><a
											href="/webtest/menus/getMenus?nowpage=${nowpage-1 }">上一页</a></li>
									</c:if>
									<c:if test="${nowpage > 3 }">
										<c:if test="${totalPage-nowpage < 3 }">
											<c:forEach var="i" begin="${nowpage-3 }" end="${totalPage }">
												<li><a href="/webtest/menus/getMenus?nowpage=${i }">${i }</a>
												</li>
											</c:forEach>
										</c:if>
										<c:if test="${totalPage-nowpage >= 3 }">
									...
									<c:if test="${hasNext == true }">
										<li><a
											href="/webtest/menus/getMenus?nowpage=${nowpage+1 }">下一页</a></li>
									</c:if>
								</ul>
							</div>
						</div>
		...
		function del(e) {
			...
			if (confirm("确定删除？")) {
				$
						.ajax({
							type : "get",
							url : "/webtest/menus/deleteMenu?id=" + delid,
							dataType : "json",
							success : function(result) {
								if (result != null && result != "") {
									document.forms[0].action = "/webtest/menus/getMenus";
