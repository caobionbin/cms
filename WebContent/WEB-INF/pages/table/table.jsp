				...
				<div class="modal  fade" id="addModal" role="dialog" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<form action="/webtest/tables/addTables" method="post" class="form-horizontal">
								<div class="modal-body">
								...
								<tbody>
									<c:forEach var="table" items="${tables }">
										<tr>
											...
											<td>
												<a class="btn btn-info" href="/webtest/tables/getUpdateTable?id=${table.id }"><i class="glyphicon glyphicon-pencil"></i> 修改</a>
						...
						<div class="col-md-12">
							...
							<ul class="pagination">
								<c:if test="${hasPre == true}">
									<li><a href="/webtest/tables/getTables?nowpage=${nowpage-1 }">上一页</a></li>
								</c:if>
								<c:if test="${nowpage > 3 }">
									<c:if test="${totalPage-nowpage < 3 }">
										<c:forEach var="i" begin="${nowpage-3 }" end="${totalPage }">
											<li>
												<a href="/webtest/tables/getTables?nowpage=${i }">${i }</a>
											</li>
										</c:forEach>
									</c:if>
									<c:if test="${totalPage-nowpage >= 3 }">
								...
								<c:if test="${hasNext == true }">
									<li><a href="/webtest/tables/getTables?nowpage=${nowpage+1 }">下一页</a></li>
								</c:if>
							</ul>
						</div>
						</div>
